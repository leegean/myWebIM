(function(win, doc){
	...
	//	countDownButton :  $('mVerifyBtn'), 
		...
		bindEvent: function(){
			...
			addEvent(that.errorDialogBtnT,'click',function(){
				...
				opt.onsuccess = function(ret){
					var result = parseJSON(ret);
					if(result.retcode == 20000000){
						var href =  'https://passport.weibo.cn/signin/loginsms?code='+result.data.code+'&r='+LOGIN_SUCCESS_ADDRESS+'&rf='+loginRFValue+'&'+formstr;
							...
							ucORweiboLogin.onclick = function(){
								ajax({
									...
									onsuccess: function(ret){
										var result = parseJSON(ret);
										if(result.retcode == 20000000){
											win.location.href = "sinaweibo://browser?url=https%3A%2F%2Fpassport.weibo.cn%2Fwapclient%2Fconfirm%3Ff%3Dw%26token%3D"+result.data.token;
		...
		dealLoginFail: function(result){
			...
			var that = this;
			var loginRFValue = that.loginRF.value;
			if(result.retcode == 50011009 || result.retcode == 50011011 || result.retcode == 50011002 || result.retcode == 50011008 || 
					result.retcode == 50011010 || result.retcode == 50011012){
				//当rf为1的时候只提示错误
